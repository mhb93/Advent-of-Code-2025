...
// This will be just like part 1, except we remove rolls as we go
        ...
        for(int lineIndex = 0; lineIndex < board[boardIndex].length(); lineIndex++){
            if(board[boardIndex][lineIndex] == '@'){
                ...
                if(surroundingRolls < 4){
                    ...
                    moveableRolls++;
                    oldMoveables++;
                    board[boardIndex][lineIndex] = '.';
                    cout << "Now the line looks like this:\n" << board[boardIndex] << "\n" << endl;
                }
                surroundingRolls = 0;
            }
        }
...
main(){
    ...
    int nextRound = countMoveableRolls(board);
    int numberOfMoveableRolls = nextRound;
    
    while(nextRound != 0){
        nextRound = countMoveableRolls(board);
        numberOfMoveableRolls += nextRound;
    }

    cout << "\nNow the board looks like this:" << endl;
